...
class ChopsticksModel:
    ...
    def get_player_hands(self, player_id: PlayerId) -> List[HandInfo]:
        ...
        return cast(List[HandInfo], sorted([h for h in self.hands[player_id]], key=lambda h: h.hand_id))
    ...
    def get_split_sources(self) -> List[HandInfo]:
        ...
        return cast(List[HandInfo], [h for h in self.get_player_hands(self.current_player_id) 
    ...
    def get_tap_sources(self) -> List[HandInfo]:
        ...
        return cast(List[HandInfo], [h for h in self.get_player_hands(self.current_player_id) if h.is_active()])
    ...
    def get_tap_targets(self) -> List[HandInfo]:
        ...
        return cast(List[HandInfo], sorted(targets, key=lambda h: (h.player_id, h.hand_id)))
    ...
    def do_tap(self, source: HandId, target: tuple[PlayerId, HandId]) -> bool:
        ...
        self.hands[target_player] = [
            cast(HandInfoImpl, new_target_hand) if h.hand_id == target_hand_id else h
            for h in self.hands[target_player]
        ]
        self._next_player()
        return True
    ...
    def do_split(self, source: HandId, target: HandId, to_transfer: int) -> bool:
        ...
        self.hands[self.current_player_id] = [
            cast(HandInfoImpl, new_source_hand) if h.hand_id == source else
            cast(HandInfoImpl, new_target_hand) if h.hand_id == target else h
            for h in self.hands[self.current_player_id]
        ]
        self._next_player()
        return True
